...
function buildPrompt(
  ...
  lines.push('  - 宵夜：22:00–00:00（可選）')
  lines.push('    ※ 用戶提供的宵夜餐廳：arrivalTime 必須 >= 22:00，正餐（full_meal）也允許')
  lines.push('    ※ AI 推薦的宵夜地點：僅限小吃（snack），最多 1~2 個')
  lines.push('    ⚠ 嚴格禁止：將用戶設定為「宵夜」的餐廳安排在 22:00 之前——即使餐廳 17:30 已開始營業，也絕對不可提前至晚餐時段安排')
  lines.push('【餐廳不足時的處理】')
